class QuranConstants {
  ...
  static List<Map<String, dynamic>> juzList(List<Map<String, dynamic>> surahs) {
    return List.generate(30, (index) {
      int juz = index + 1;
      int page = juzToPage[juz]!;
      return {
        'id': juz,
        'name': 'Cüz $juz',
        'page': page,
  ...
  static String getMushafUrl(int page) {
    // BetimShala Quran Images API - Stable and reliable
    // Pattern: 1.png, 2.png ... 604.png
  ...
  static int getJuzByPage(int page) {
    for (int i = 30; i >= 1; i--) {
      if (page >= (juzToPage[i] ?? 0)) return i;
    }
    return 1;
  }

  static int getSurahByPage(int page) {
    int currentSurah = 1;
    for (var entry in surahToPage.entries) {
      if (page >= entry.value) {
        currentSurah = entry.key;
      } else {
        break;
      }
    }
